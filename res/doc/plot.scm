...
(loop with amp = .7
  ...
  (plot :title "My Notes"
        ...
        :style "pianoroll"              ; display as horizontal boxes
        ))
...
(define (myhook x y)
  (let* ((spec (fm-spectrum y (between 1.5 3.0) 3))
         (keys (spectrum-keys spec :quant 1 :unique #t :min (- y 17) :max (+ y 17))))
...
(plot :title "my fm chords" :xaxis '(0 10) :yaxis "keynum")
...
(begin
  ...
  (define (arpa-harmonic fundnote dur gap)
    ...
    (let ((result (list)))
      ...
      (loop with fund = (hz fundnote) and time = 0
