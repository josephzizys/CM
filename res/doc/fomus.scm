...
(define (examp2 )
  (process with marks = '(#f (".") (">"))
           repeat 21
           do (send "fms:note" :dur (if (< (elapsed) 10) 1/2 1)
...
(define (examp8-marks )
  (process repeat 8
           do (send "fms:mark" :part (odds .5 "0" "1")
                    :time (random 20.0)
                    :marks '("^"))))
...
(define (examp9 )
  (process for o from 0 to 20 by 1/2 
           do (send "fms:note" :time o :dur 1/2
...
(define (examp11-notes p)
  (process for n in '(60 62 64)
           for o from 0
           do (send "fms:note" :part p :dur 1 :pitch n)
           (wait 1)))
...
(define (play-harmonics note beat harm1 harm2 pname)
  (process for harm from harm1 to harm2
           for knum = (key (* (hz note) harm))
           do (send "fms:note" :dur (rhythm beat) :pitch knum :part pname)
           (wait (rhythm beat))))
