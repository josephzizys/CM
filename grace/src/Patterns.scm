...
(define-constant +constant-weights+ (arithmetic-shift 1 2))  ; avoid random index recalc
(define-constant +count-periods+    (arithmetic-shift 1 3))  ; period counts subperiods
(define-constant +count-values+     (arithmetic-shift 1 4))  ; period counts values
(define-constant +depth-first+      (arithmetic-shift 1 5))  ; pattern moves on eop
(define-constant +breadth-first+    (arithmetic-shift 1 6))  ; pattern moves each time
...
(define (pperiod obj out)
  (fprintf out "#<period ~S ~S ~S ~S ~S ~S>"
	   (period-count obj) (period-length obj) (period-stream obj)
	   (period-default obj) (period-omit obj) (period-reps obj)
	   ))

(define-record pattern flags data length datum period value state repeat returning counting traversing next mapr cache)

(define (ppattern obj out)
  (fprintf out "#<pattern ~S ~S ~S ~S ~S ~S ~S ~S ~S ~S>"
	   (pattern-flags obj) (pattern-data obj) (pattern-length obj)
	   (pattern-datum obj) (pattern-period obj) (pattern-value obj)
	   (pattern-state obj) (pattern-repeat obj) 
	   (pattern-returning obj)
	   (pattern-cache obj)
	   ))
...
(define (initialize-pattern obj data for rep flags len dper getr mapr)
  ...
  (let ((con? #t))
    (map-pattern-data (lambda (x) (if (pattern? x) (set! con? #f))) 
		      obj)
    (if con? (set! flags (bitwise-ior flags +constant-data+))))
  ;; parse counting option
  (let ((counting (pattern-counting obj)))
    (case counting
      ((#:periods )
       (set! flags (bitwise-ior flags +count-periods+)))
      ((#:values )     
       (set! flags (bitwise-ior flags +count-values+)))
      (else
  ...
  (let ((traversing (pattern-traversing obj)))
    (case traversing
      ((#:depth-first ) 
       (set! flags (bitwise-ior flags +depth-first+)))
      ((#:breadth-first )
       (set! flags (bitwise-ior flags +breadth-first+)))
      (else
  ...
  ;; dynamically.
  (cond ((bitwise-test flags +count-values+)
	 (set! flags (bitwise-and flags (bitwise-not +count-periods+))))
	(else
	 (if (bitwise-test flags +constant-data+)
	     (set! flags (bitwise-ior 
			  (bitwise-and 
			   flags (bitwise-not +count-periods+))
			  +count-values+))
	     (set! flags (bitwise-ior flags +count-periods+)))))
  ...
  (let ((per (or for dper)))
    ...
    (when (not for)
      (set! flags (bitwise-ior flags +default-period+)))
	 ...
	 (let ((period (pattern-period obj))
	   ...
	   (if nomore
		 ...
		 (if (bitwise-test flags +constant-data+)
		     ...
		     (do ((dyn? (and (bitwise-test flags +count-periods+)
				     (eq? (period-length period) #t)))
			  (stop #f))
			 (stop #f)
		       ;; increment over 0 length substreams
		       (do ()
			   ((not (eq? (pattern-datum obj) +nad+)) #f)
		 ...
		 (cond ((eq? state +eop+)
			...
			(set! state #f))
		       (else
			(if (bitwise-test flags +breadth-first+)
			    (pattern-datum-set! obj +nad+))
			(if (bitwise-test flags +count-values+)
...
(define (reset-period obj)
  (if (not (pattern? obj)) 1
	...
	(when (and dyn
		   (bitwise-test (pattern-flags obj) +count-periods+))
	  (let ((zeros 0))
	    (map-pattern-data
	     (lambda (x)
...
(define (ppalin obj port)
  (fprintf port "#<palin pos: ~S: len: ~S inc: ~S mode: ~S ~S>"
  ...
  (let* ((cache (pattern-cache obj))
    ...
    (cond ((< pos 0 ) 
	   ...
	   (let ((m (next-1 (palin-elide cache)))
	     ...
	     (if (or (eq? m #t) (and (pair? m) (eq? (car m) #t)))
		 ;; -2 marks very first call, dont skip inital element
		 (if i (set! pos 0) (set! pos 1))
		 (set! pos 0))	     
	     (if (bitwise-test (pattern-flags obj) +default-period+)
	  ...
	  ((= pos (palin-len cache))
	   ...
	   (let ((m (palin-mode cache)))
	     ...
	     (if (or (eq? m #t) (and (pair? m) (pair? (cdr m))
				     (eq? (cadr m) #t)))
		 (set! pos (- pos 2))
		 (set! pos (- pos 1))))
	   ))
...
(define (prandom-item obj port)
  (fprintf port "#<random-item: ~S ~S ~S ~S ~S ~S ~S ~S>"
	   (random-item-datum obj) (random-item-index obj) (random-item-weight obj)
	   (random-item-min obj) (random-item-max obj) (random-item-count obj)
	   (random-item-id obj)	(random-item-minmax obj)))
...
(define (make-weighting data . args)
  (let* ((pool (canonicalize-weighting-data data))
    ...
    (if const-weight (set! flags (bitwise-ior flags +constant-weights+)))
...
(define (recalc-weightings obj fix?)
  (let ((data (car (pattern-data obj)))
	...
	(do ((tail data (cdr tail))
	  ...
	  (set! index (inexact->exact
		       (floor (* (/ (random-item-index (car tail))
				    range)
...
(define (next-in-weighting obj)
  ...
  (let* ((pool (pattern-data obj))
	 ...
	 (flags (pattern-flags obj))
         (last (cdr pool)))
    (unless (bitwise-test flags +constant-weights+)
  ...
  (let* ((graph (pattern-data obj))
	...
	(let ((link (next-1 (graph-node-to this)))
	  ...
	  (do ((tail nodes (cdr tail)))
	    ...
	    (if (eq? link (graph-node-id (car tail)))
		(set! next (car tail))))))))
