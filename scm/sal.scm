...
(begin
  ...
  (define SalPlotHook #x3e00)
  (define SalPrint #x3f00)
  (define SalRun #x4010)
  (define SalSend #x4100)
  (define SalSet #x4200)
  (define SalSprout #x4300)
  (define SalSystem #x4400)
  (define SalUse #x4500)
  (define SAL_COMMAND_END #x4600)
  (define SAL_CONSTITUENT_BEG #x4600)
  (define SalElse #x4700)
  (define SalEnd #x4811)
  (define SalOutput #x4900)
  (define SalReturn #x4a00)
  (define SalThen #x4b00)
  (define SalUnless #x4c00)
  (define SalUntil #x4d00)
  (define SalWait #x4e00)
  (define SalWhen #x4f00)
  (define SalWhile #x5000)
  (define SalWith #x5100)
  (define SAL_CONSTITUENT_END #x5200)
  (define SAL_STATEMENT_END #x5200)
  (define SAL_CLAUSAL_BEG #x5200)
  (define SalAbove #x5300)
  (define SalBelow #x5400)
  (define SalBy #x5500)
  (define SalDownto #x5600)
  (define SalFinally #x5700)
  (define SalFor #x5800)
  (define SalFrom #x5900)
  (define SalIn #x5a00)
  (define SalOver #x5b00)
  (define SalRepeat #x5c00)
  (define SalTo #x5d00)
  (define SAL_CLAUSAL_END #x5e00)
  (define SAL_LITERAL_END #x5e00)
  (define SAL_DEFINE_BEG #x5e00)
  (define SalFunction #x5f00)
  (define SalProcess #x6000)
  (define SalVariable #x6100)
  (define SAL_DEFINE_END #x6200)
  (define SAL_IDENTIFIER_BEG #x6200)
  (define SalIdentifier #x6300)
  (define SalSlotRef #x6400)
  (define SAL_IDENTIFIER_END #x6500)
  (define SAL_TOKEN_END #x6500)
  (define SAL_RULE_BEG #x6500)
  (define SalNumberRule #x6600)
  (define SalBoolRule #x6700)
  (define SalAtomRule #x6800)
  (define SalListRule #x6900)
  (define SalEltRule #x6a00)
  (define SalArefRule #x6b00)
  (define SalIfExprRule #x6c00)
  (define SalUnquoteRule #x6d00)
  (define SalFuncallRule #x6e00)
  (define SalFunargsRule #x6f00)
  (define SalPargsRule #x7000)
  (define SalKargsRule #x7100)
  (define SalOpRule #x7200)
  (define SalMexprRule #x7300)
  (define SalTermRule #x7400)
  (define SalSexprRule #x7500)
  (define SalBindingsRule #x7600)
  (define SalBindRule #x7700)
  (define SalAssignmentRule #x7800)
  (define SalAssignRule #x7900)
  (define SalAssignerRule #x7a00)
  (define SalSetRule #x7b00)
  (define SalFunctionReturnRule #x7c00)
  (define SalProcessWaitRule #x7d00)
  (define SalBlockRule #x7e00)
  (define SalConditionalRule #x7f00)
  (define SalLoopStatementRule #x8000)
  (define SalRunStatementRule #x8100)
  (define SalSteppingRule #x8200)
  (define SalTerminationRule #x8300)
  (define SalPrintStatementRule #x8400)
  (define SalExecStatementRule #x8500)
  (define SalOpenStatementRule #x8600)
  (define SalSendStatementRule #x8700)
  (define SalSproutStatementRule #x8800)
  (define SalOutputStatementRule #x8900)
  (define SalLoadStatementRule #x8a00)
  (define SalSystemStatementRule #x8b00)
  (define SalChdirStatementRule #x8c00)
  (define SalPlayStatementRule #x8d00)
  (define SalPlotStatementRule #x8e00)
  (define SalPlotHookStatementRule #x8f00)
  ...
  (define SAL_RULE_END #x9800)
  (define SAL_TYPE_END #x9800)
  )
...
(defrule SalPlotHookStatementRule
  (and SalPlotHook SalSexprRule SalComma SalSexprRule (* SalComma SalSexprRule))
  (lambda (args errf)
    ;; (<plothook> <title> <,> <hook> (<,> <arg> ...) )
    (make-parse-unit
     SalPlotHookStatementRule
     (cons (first args)
	   (cons (second args)
		 (cons (fourth args)
		       (remove-token-type (fifth args)
					  SalComma))))
     #f))
  (simple-unit-emitter 'plot-hook)
  )
...
(define (sal str . args)
  (with-optkeys (args (tokens '()) (expand? #f)
		      (rule SalStatementRule))
    ;; tokens is () or list of token lists ((...) ...)
    (if (or (null? tokens) (not tokens))
	(set! tokens (tokenize str))
	(set! tokens (tokenize tokens)))
    (if (not (null? tokens))
...
(define (tokenize input)
  ;; if input is a string then call the C tokenizer and convert to a
  ;; token list otherwise just convert the list to tokens
  (if (string? input)
      (let ((res (ffi_sal_tokenize input)))
	(set! input (read-from-string res))))
  (if (pair? input)
      (map list->token input)
      (list)))
	    ...
	    (lambda (thing lev quo)
	      (cond ((not thing)
		    ...
		    ((eq? thing #t)
		     (print-output "#t" ))
		    ((number? thing)
		     (print-output (number->string thing))
;;		     (if (inexact? thing)
...
;;			 (print-output (number->string thing)))
		    ...
		    ((string? thing)
		     (print-output thing quo)
		     )
		    (else
		     (print-output (format #f "~S" thing)))))))
    (do ((tail args (cdr tail)))
	((null? tail)
	 (print-output "\n"))
      (printer (car tail) 0 #f))
    (values)))
  ...
  (define (runtests rule tests nums)
	...
	((null? tail) (values))
	  ...
	  (let* ((test (car tail))
	    ...
	    (format #t "~%test[~A] ~S :~%" (+ i 1) name)
	    (let ((form (parse rule toks #f 0 #f #f)))
	      (if (parse-error? form)
		  (format #t "ERROR ~S~%" form)
		  (format #t "=> ~S~%" form)))))))
