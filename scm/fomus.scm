...
;; (define (fms:filter sets . keys)
;;   (do ((s sets (cddr s))
;;        (r '()))
;;       ((null? s) (reverse! r))
;;     (unless (member (car s) keys)
;; 	    (set! r (cons (cadr s) (cons (car s) r))))))
...
(define (fms:entry time dur part voice grace marks sets)
  ...
  (when marks
	(do ((e marks (cdr e)))
	    ((null? e))
	  (let ((args (let ((x (car e))) (if (list? x) x (list x)))))
...
(define (fms:note . args)
  (with-optkeys (args (time 0) (dur 1) (pitch 60) part (voice 1) (dyn 0.5) grace (marks '()) (sets '()) &allow-other-keys)
		...
		(unless (or (list? marks) (eq? marks #f)) (ffi_fms_err) (error "expected a list for marks argument"))
...
(define (fms:setting set val . append)
  (unless (or (string? set) (symbol? set)) (ffi_fms_err) (error "expected a string or symbol for filename argument"))
...
(define (fms:open-score filename . args)
  ...
  (with-optkeys (args (percinsts '()) (insts '()) (parts '()) (metaparts '()) (measdefs '()) (sets '()) (clear #t) new (run #t) &allow-other-keys)
		...
		(map (lambda (x) (apply fms:percinst x)) percinsts)
		(map (lambda (x) (apply fms:inst x)) insts)
		(map (lambda (x) (apply fms:part x)) parts)
		(map (lambda (x) (apply fms:metapart x)) metaparts)
		(map (lambda (x) (apply fms:measdef x)) measdefs)
